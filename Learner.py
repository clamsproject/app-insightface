...
#             self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=40, verbose=True)
    ...
    def load_state(self, conf, fixed_str, from_save_folder=False, model_only=False):
        if from_save_folder:
            ...
            save_path = conf.model_path            
        #self.model.load_state_dict(torch.load(save_path/'model_{}'.format(fixed_str))) # jinny
        self.model.load_state_dict(torch.load(save_path/'model_{}'.format(fixed_str), map_location=torch.device('cpu')))
        if not model_only:
            #self.head.load_state_dict(torch.load(save_path/'head_{}'.format(fixed_str))) # jinny
            self.head.load_state_dict(torch.load(save_path/'head_{}'.format(fixed_str), map_location=torch.device('cpu')))
            #self.optimizer.load_state_dict(torch.load(save_path/'optimizer_{}'.format(fixed_str))) # jinny
